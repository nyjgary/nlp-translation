...
def evaluate_attn(model, loader, src_id2token, targ_id2token, teacher_forcing_ratio): 
    """ Evaluates a model given a loader, id2token dicts, and teacher_forcing_ratio. 
        Outputs avg loss, avg bleu, as well as indices and tokens representing source, reference, and model translations. 
...
def train_and_eval_attn(model, loaders_full, loaders_minibatch, loaders_minitrain, params, vocab, 
    lazy_eval=True, print_intermediate=True, save_checkpoint=True, save_to_log=True, inspect_samples=None): 
        ...
        for batch, (src_idxs, targ_idxs, src_lens, targ_lens) in enumerate(train_loader_):
            ...
            if batch % 1000 == 0 or ((epoch==num_epochs-1) & (batch==len(train_loader_)-1)):
                ...
                result['val_loss'], result['val_bleu'], val_hyp_idxs, val_ref_idxs, val_source_idxs, val_hyp_tokens, val_ref_tokens, val_source_tokens, attn = \
                    evaluate_attn(model, dev_loader_, src_id2token, targ_id2token, teacher_forcing_ratio=teacher_forcing_ratio)
                # calculate metrics on train set (or proxy thereof) only if lazy_eval not set to True 
                if not lazy_eval: 
                    result['train_loss'], result['train_bleu'], train_hyp_idxs, train_ref_idxs, train_source_idxs, train_hyp_tokens, train_ref_tokens, train_source_tokens, attn = \
                            evaluate_attn(model, train_loader_proxy, src_id2token, targ_id2token, teacher_forcing_ratio=teacher_forcing_ratio)
                else: 
                    result['train_loss'], result['train_bleu'] = 0, 0 
                ...
                if inspect_samples is not None: 
                    ...
                    if not lazy_eval: 
                        print("Sampling from training predictions...")
                        sample_predictions_attn(train_hyp_idxs, train_ref_idxs, train_source_idxs, 
                            train_hyp_tokens, train_ref_tokens, train_source_tokens, targ_id2token, attn, num_samples=inspect_samples)
                    # sample predictions from validation set 
                    print("Sampling from val predictions...")
                    sample_predictions_attn(val_hyp_idxs, val_ref_idxs, val_source_idxs, 
                        val_hyp_tokens, val_ref_tokens, val_source_tokens, targ_id2token, attn, num_samples=inspect_samples)
...
def sample_predictions_attn(hyp_idxs, ref_idxs, source_idxs, hyp_tokens, ref_tokens, source_tokens, id2token, attn, num_samples=1): 
